...
* Most settings on the `Admin` page are fairly self-explanatory. `Extra days per open challenge:` is only applicable when `Simultaneous challenges allowed:` is checked. This is the extra time a challengee gets for each of his/her current open challenges when s/he gets a new challenge. `Page title` is part of the title and the main header of each page, and part of the subject line of each challenge email sent.
